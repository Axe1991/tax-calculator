...
st.set_page_config(page_title="Tax Calculator - Detailed Salary", layout="centered")
st.title("🧮 Income Tax Calculator with Detailed Salary (FY 2025-26)")
...
with st.form("salary_form"):
    ...
    if claim_hra:
        ...
        hra_exemption = min(hra_received, rent_paid - 0.1 * basic_salary, percent_salary * basic_salary)
        if hra_exemption < 0:
            hra_exemption = 0
...
if submitted:
    gross_salary = sum([basic_salary, da, hra_received, special_allowance, lta, transport_allowance, bonus, pf, variable_pay, other_allowance])
    ...
    if claim_hra:
        hra_1 = hra_received
        hra_2 = rent_paid - 0.1 * basic_salary
        hra_3 = percent_salary * basic_salary
        hra_2 = hra_2 if hra_2 > 0 else 0
        st.markdown("**📘 HRA Exemption Calculation:**")
        st.write(f"1. Actual HRA Received: ₹{hra_1:,.2f}")
        st.write(f"2. Rent Paid – 10% of Basic Salary: ₹{hra_2:,.2f}")
        st.write(f"3. {'50%' if is_metro else '40%'} of Basic Salary: ₹{hra_3:,.2f}")
